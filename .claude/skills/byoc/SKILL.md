...
If the user is starting from scratch (no canister yet), use the `build-prometheus-icp-mcp-server` skill instead — there ICForge creates and deploys the canister, so no cycles or manual canister creation are needed (ICForge is pay-as-you-go: a $10 initial top-up at signup, with managed cycles billed against the balance). This skill never requires cycles: registration calls are ordinary signed calls that cost the caller nothing.
...
3. Top up their ICForge account ($10 initial top-up for new accounts — pay-as-you-go; deploys and managed cycles are billed against the balance)
4. Connect the repo
